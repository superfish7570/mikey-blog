...
export class OrgManagementClient {
  ...
  public async getAllOrgs(page: number = 1, limit: number = 10) {
    const { count } = await this.httpClient.request({
      method: 'GET',
   ...
   */
  public async getOrgByTenantId(tenantId: string) {
    const url = `${this.options.host}/api/v2/orgs/by-tenant?tenant_id=${tenantId}`;
    const data = await this.httpClient.request({
      method: 'GET',
      url
    });
